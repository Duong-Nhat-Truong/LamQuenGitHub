...
void GhiVaoFile(struct SinhVien ds[], int n){
    char TenFile[100];
    printf("\nNhap vao duong dan va ten file: ");
    fflush(stdin);
    fgets(TenFile, sizeof(TenFile), stdin);
    XoaDauXuongDong(TenFile);

    FILE *fp;
    fp = fopen(TenFile,"wb");
    if(fp == NULL){
        printf("\n*** LOI KHONG THE TAO HOAC MO FILE ***\n");
        return;
    }
    fwrite(&n, sizeof(n), 1, fp);
    for(int i=0; i<n; i++){
        fwrite(&ds[i], sizeof(struct SinhVien), 1, fp);
    }
    fclose(fp);
}

void DocTuFile(struct SinhVien ds[], int *n){
    char TenFile[100];
    printf("\nNhap vao duong dan va ten file: ");
    fflush(stdin);
    fgets(TenFile, sizeof(TenFile), stdin);
    ...
    for(int i=0; i<*n; i++){
        fread(&ds[i], sizeof(struct SinhVien), 1, fp);
    }
    fclose(fp);
...
int main(){
    ...
    do{
        ...
        printf("Nhan phim 12: Ghi danh sach sinh vien vao File\n");
        printf("Nhan phim 13: Doc danh sach sinh vien tu File\n");
